...
// Sits in front of the country media bucket public URL.
...
const FALLBACK_R2_PUBLIC_BASE = 'https://media.mansiondeseo.com';
...
export default {
  async fetch(request, env) {
    ...
    const publicBase = String(env?.R2_PUBLIC_BASE || env?.R2_PUBLIC_URL || FALLBACK_R2_PUBLIC_BASE).replace(/\/+$/, '');
    const r2Url = `${publicBase}${url.pathname}`;
    return fetch(r2Url, {
