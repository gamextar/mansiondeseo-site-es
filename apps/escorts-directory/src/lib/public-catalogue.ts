...
const commonSelect = `
  SELECT p.slug, p.display_name, p.city_slug, p.city_name, p.short_bio, p.contact_url, p.contact_label,
...
export async function getPublicListings(env: Record<string, any>, city = '', limit = 48) {
  ...
  const result = city
    ? await env.DB.prepare(query).bind(city, limit).all()
...
export async function getPublicListing(env: Record<string, any>, slug: string) {
  ...
  const result = await env.DB.prepare(`${commonSelect} AND p.slug = ? LIMIT 1`).bind(slug).first();
...
export async function getPublicCities(env: Record<string, any>) {
  ...
  const result = await env.DB.prepare("SELECT city_slug AS slug, city_name AS name, COUNT(*) AS count FROM escort_profiles WHERE status = 'published' GROUP BY city_slug, city_name ORDER BY count DESC, name ASC").all();
