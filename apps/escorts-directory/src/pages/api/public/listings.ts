...
export const GET: APIRoute = async ({ request, locals }) => {
  ...
  const query = `
    SELECT p.slug, p.display_name, p.city_slug, p.city_name, p.short_bio,
    ...
    WHERE p.status = 'published' ${cityClause}
    ORDER BY CASE tier ${tierOrder} ELSE 0 END DESC, p.published_at DESC
    LIMIT 48
  `;
  const result = city ? await db.prepare(query).bind(city).all() : await db.prepare(query).all();
