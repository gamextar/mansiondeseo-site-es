...
export default function DesktopSidebar() {
  ...
  const sidebarRingPercent = Math.max(1, Math.min(18, Number(siteSettings?.sidebarStoryRingWidth ?? 4)));
    ...
    <aside className="hidden lg:flex fixed left-0 top-0 bottom-0 w-64 xl:w-72 z-40 flex-col bg-mansion-card/50 border-r border-mansion-border/30 backdrop-blur-xl">
      ...
      {user && (
        <Link to="/perfil" className="flex flex-col items-center py-6 border-b border-mansion-border/20 hover:opacity-90 transition-opacity">
          <div className={`relative w-[60%] aspect-square rounded-full ${user.has_active_story ? 'bg-gradient-to-tr from-mansion-gold via-mansion-crimson to-mansion-gold' : 'bg-gradient-to-br from-mansion-gold/60 to-mansion-gold-light/40'}`}>
