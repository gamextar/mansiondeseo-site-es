...
function filterViewerStories(stories = [], viewerId = '') {
  const currentUserId = String(viewerId || '').trim();
  const list = Array.isArray(stories) ? stories : [];
...
function getCachedHomeStoriesEntry(userId, isDesktopViewport, limit, { allowStale = false, roleKey = 'all' } = {}) {
  try {
    ...
    const isFresh = timestamp > 0 && ageMs <= HOME_STORIES_CACHE_TTL_MS;
    if (!allowStale && !isFresh) return null;
    const stories = Array.isArray(parsed?.stories) ? parsed.stories : [];
    if (stories.length === 0) return null;
    return {
      stories: stories.slice(0, Math.max(1, Number(limit) || stories.length)),
      timestamp,
      ageMs,
      isFresh,
    };
...
function clearCachedHomeStories() {
  try {
    for (let index = localStorage.length - 1; index >= 0; index -= 1) {
      const key = localStorage.key(index);
      if (key?.startsWith(HOME_STORIES_CACHE_PREFIX)) localStorage.removeItem(key);
    }
  } catch {}
}
...
export default function FeedPage({ initialData }) {
  ...
  const initialProfileRetryRef = useRef(0);
  ...
  const loadProfiles = useCallback(({ forceFresh = false, cursor = 0, pageSize, targetPageCursor } = {}) => {
    ...
    return fetchProfilesBlock({ forceFresh, cursor, pageSize })
      .then(data => {
        ...
        initialProfileRetryRef.current = 0;
        applyLoadedProfiles({
          data: data.data,
          cursor,
      ...
      .catch(() => {
        ...
        const canRetry = isInitialBlock && !hasVisibleProfiles && initialProfileRetryRef.current < 2;
        if (canRetry) {
          initialProfileRetryRef.current += 1;
          keepLoadingForRetry = true;
          window.setTimeout(() => {
            loadProfiles({ forceFresh: true, cursor: 0, pageSize, targetPageCursor: 0 });
          }, 1200 * initialProfileRetryRef.current);
          return;
        }
      ...
      .finally(() => {
        if (myId === loadIdRef.current && !keepLoadingForRetry) setLoading(false);
      });
  ...
  const bootstrapStoryProfiles = useMemo(
    () => mapStoriesToRailProfiles(filterViewerStories(bootstrapStories, user?.id)).slice(0, storyLimit),
    [bootstrapStories, storyLimit, user?.id]
  );
  ...
  useEffect(() => {
    ...
    setHomeStories((current) => {
      const filtered = filterViewerStories(current, user.id);
      return filtered.length === current.length ? current : filtered;
    });
    setBootstrapStories((current) => {
