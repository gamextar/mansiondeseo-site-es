...
function RankCard({ profile, compact = false }) {
  ...
  return (
    <Link
      ...
      className={`group relative overflow-hidden rounded-[1.75rem] border border-mansion-border/25 bg-mansion-card/60 backdrop-blur-xl transition-all hover:-translate-y-0.5 hover:border-mansion-gold/30 hover:bg-mansion-card/80 ${compact ? 'p-4' : 'p-5'}`}
    >
      <div className={`pointer-events-none absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${rankAccent(profile.rank)}`} />
      <div className="flex items-start gap-4">
        <div className="relative shrink-0">
          <div className={`${compact ? 'h-16 w-16' : 'h-20 w-20'} overflow-hidden rounded-[1.35rem] border border-white/10 bg-mansion-elevated`}>
            {profile.avatar_url ? (
          ...
          <div className="absolute -left-2 -top-2 inline-flex h-8 min-w-8 items-center justify-center rounded-full border border-mansion-gold/30 bg-black/60 px-2 text-xs font-bold text-mansion-gold">
            #{profile.rank}
          </div>
        </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs text-text-dim">{profile.role}</p>
            </div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 text-xs">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-mansion-gold/20 bg-mansion-gold/10 px-2.5 py-1 text-mansion-gold">
              <Eye className="h-3.5 w-3.5" />
              {Number(profile.visits_total || 0).toLocaleString('es-AR')} visitas
            </span>
            {location && (
          ...
          <>
            ...
            <section className="mt-8">
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <p className="text-xs text-text-dim">{profiles.length} perfiles</p>
              </div>
