...
export default function VideoFeedPage() {
  ...
  const arrowScrollAnimRef = useRef(0);
  ...
  useEffect(() => () => {
    if (arrowScrollAnimRef.current) {
      cancelAnimationFrame(arrowScrollAnimRef.current);
    }
  }, []);
  ...
  const animateScrollTo = useCallback((targetTop, duration = 520) => {
    const container = containerRef.current;
    if (!container) return;
    ...
    const step = (now) => {
      ...
      if (progress < 1) {
        arrowScrollAnimRef.current = requestAnimationFrame(step);
      } else {
        arrowScrollAnimRef.current = 0;
      }
    };
  ...
  const scrollByOne = useCallback((dir) => {
    ...
    animateScrollTo(container.scrollTop + dir * container.clientHeight);
  }, [animateScrollTo]);
  ...
  const scrollByArrow = useCallback((dir) => {
    ...
    const rawIndex = Math.round(container.scrollTop / container.clientHeight);
    const targetIndex = Math.max(0, Math.min(rawIndex + dir, infiniteStories.length - 1));
    animateScrollTo(targetIndex * container.clientHeight, 720);
  }, []);
